...
struct AEGfxTexture* DrawObject::GetTexture() const
{
	return m_px_Texture;
}

AEGfxVertexList* DrawObject::GetVertexList() const
{
	return m_px_Obj;
}
...
{
	SetPositionX(drawObj.GetPosition().x);
	SetPositionY(drawObj.GetPosition().y);
	m_px_Texture = drawObj.GetTexture();
	m_f_Rotation = drawObj.GetRotation();
	m_f_SizeX = drawObj.GetSizeX();
	m_f_SizeY = drawObj.GetSizeY();

	size_t nameLen = strlen(drawObj.GetName()) + 1;
	m_c_Name = new char[nameLen];
	strcpy_s(m_c_Name, nameLen, drawObj.GetName());

	nameLen = strlen(drawObj.GetTexName()) + 1;
	m_c_TextureName = new char[nameLen];
	strcpy_s(m_c_TextureName, nameLen, drawObj.GetTexName());

	AEGfxMeshStart();
	
	m_px_Obj = drawObj.GetVertexList();
